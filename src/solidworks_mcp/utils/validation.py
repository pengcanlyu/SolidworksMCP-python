...
async def _validate_solidworks_installation(config: SolidWorksMCPConfig) -> None:
    ...
    # Check COM registration without instantiating the COM server.
    # win32com.client.Dispatch("SldWorks.Application") starts SolidWorks in
    # embedding mode during MCP startup, which is too invasive for validation.
    try:
        import pythoncom
